...
if (isGet()){
    ...
    if (!empty($body['portfolio_category_id'])){
        $cateId = $body['portfolio_category_id'];
...
$listAllPortfolio = getRaw("SELECT portfolios.id, portfolios.name,portfolios.user_id,portfolios.portfolio_category_id , portfolios.create_at, portfolio_categories.name as cate_name, users.fullname FROM portfolios INNER JOIN portfolio_categories ON portfolios.portfolio_category_id=`portfolio_categories`.id INNER JOIN users ON portfolios.user_id=users.id $filter ORDER BY portfolios.create_at DESC LIMIT $offset, $perPage");

//Truy vấn lấy danh sách Danh mục
$allPortfolio_categories = getRaw("SELECT id, name FROM `portfolio_categories` ORDER BY name");

// lấy dữ liệu người dùng
$listAllUsers = getRaw("SELECT * FROM users ORDER BY fullname");
        ...
        <div class="container-fluid">
                    ...
                    <div class="col-3">
                        <div class="form-group">
                            <select name="user_id" class="form-control">
                                <option value="0">Chọn Ngưởi Đăng</option>
                                <?php
            ...
            <table class="table table-bordered">
                ...
                <tr>
                    ...
                    <td>
                        <a href="?<?php echo getLinkQueryString('portfolio_category_id', $item['portfolio_category_id']); ?>"><?php echo $item['cate_name']; ?></a>
                    </td>
                    <td>
                ...
                <ul class="pagination pagination-sm">
                    ...
                    for ($index = $begin; $index <= $end; $index++) {
                        ...
                        <li class="page-item <?php echo ($index == $page) ? 'active' : false; ?>"><a class="page-link"
                                                                                                     href="<?php echo _WEB_HOST_ROOT . '?module=portfolios' . $queryString . '&page=' . $index; ?>"><?php echo $index; ?></a>
                        </li>
                    <?php } ?>
                    <?php
                    if ($page < $maxPage) {
